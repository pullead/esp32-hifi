...
void TFT_SPI::setTFTcontroller(uint8_t TFTcontroller) {
    m_TFTcontroller = TFTcontroller; // 0=ILI9341, 3=ILI9486, 5=ILI9488/ST7796, 11=ST7789V2
    ...
    if(m_TFTcontroller == ST7789V2) { m_h_res = 320; m_v_res = 170; m_rotation = 0;} // landscape dims: init() sets MADCTL_MV, like ILI9341
...
void TFT_SPI::setDisplayInversion(uint8_t i) {
    ...
    if(m_TFTcontroller == ST7789V2) { writeCommand(i ? INVON : INVOFF); }
    endWrite();
...
void TFT_SPI::displayInversion() {
    ...
    if(m_TFTcontroller == ST7789V2) { writeCommand(m_displayInversion ? INVON : INVOFF); }
}
//——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
void TFT_SPI::writeCommand(uint16_t cmd) {
    TFT_DC_LOW();
    if(m_TFTcontroller == ILI9341 || m_TFTcontroller == ILI9488_ST7796 || m_TFTcontroller == ST7789V2) spi_TFT->write(cmd);
...
uint16_t TFT_SPI::readCommand() {
    uint16_t ret = 0;
    TFT_DC_LOW();
    if(m_TFTcontroller == ILI9341 || m_TFTcontroller == ILI9488_ST7796 || m_TFTcontroller == ST7789V2) ret = spi_TFT->transfer(0);
...
void TFT_SPI::init() {
    ...
    if(m_TFTcontroller == ST7789V2) {
        if(tft_info) tft_info("init " ANSI_ESC_CYAN "ST7789V2");
        // Full vendor init sequence taken from the Waveshare ESP32-S3-Touch-LCD-1.9
        // factory program (08_FactoryProgram/main/main.c, lcd_init_cmds[]).
        // The minimal SLPOUT/COLMOD/DISPON init leaves the panel voltages
        // (VCOM/VRH/VDV) and gamma unconfigured on this module -> black screen.
        writeCommand(0x01); // SWRESET
        delay(150);
        writeCommand(MADCTL);
        spi_TFT->write(0x70); // landscape 320x170: MX|MV|ML, RGB order (factory "Rotate" variant)
        writeCommand(0x3A);   // COLMOD
        spi_TFT->write(0x55); // 16-bit RGB565
        writeCommand(0xB2);   // PORCTRL
        spi_TFT->write(0x0C); spi_TFT->write(0x0C); spi_TFT->write(0x00); spi_TFT->write(0x33); spi_TFT->write(0x33);
        ...
        spi_TFT->write(0xA4); spi_TFT->write(0xA1);
        writeCommand(0xD6);
        spi_TFT->write(0xA1);
        writeCommand(0xE0);   // PVGAMCTRL
        { const uint8_t g[14] = {0x00,0x03,0x07,0x08,0x07,0x15,0x2A,0x44,0x42,0x0A,0x17,0x18,0x25,0x27};
          for (int i = 0; i < 14; i++) spi_TFT->write(g[i]); }
        writeCommand(0xE1);   // NVGAMCTRL
        { const uint8_t g[14] = {0x00,0x03,0x08,0x07,0x07,0x23,0x2A,0x43,0x42,0x09,0x18,0x17,0x25,0x27};
          for (int i = 0; i < 14; i++) spi_TFT->write(g[i]); }
        writeCommand(INVON);  // 0x21 — factory sequence inverts (IPS panel)
        writeCommand(SLPOUT); // 0x11
        delay(120);
        writeCommand(DISPON); // 0x29
        delay(20);
        writeCommand(RAMWR);
     }
...
void TFT_SPI::setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
    ...
    if(m_TFTcontroller == ST7789V2) {
        ...
        uint16_t x1 = x + w - 1 + TFT_COL_OFFSET;
        uint16_t y0 = y + TFT_ROW_OFFSET;
        uint16_t y1 = y + h - 1 + TFT_ROW_OFFSET;
        writeCommand(CASET);
        spi_TFT->write(x0 >> 8);
        spi_TFT->write(x0 & 0xFF);
        spi_TFT->write(x1 >> 8);
        spi_TFT->write(x1 & 0xFF);
        writeCommand(RASET);
        spi_TFT->write(y0 >> 8);
        spi_TFT->write(y0 & 0xFF);
        spi_TFT->write(y1 >> 8);
        spi_TFT->write(y1 & 0xFF);
        writeCommand(RAMWR);
    }
}
//——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
