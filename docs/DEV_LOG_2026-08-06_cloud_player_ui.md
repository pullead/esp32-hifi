...
- 在线歌词已接通(见"追加 3"),播放页详情行逐句显示网易云歌词,未找到可点击重试
...
## 追加 3:在线歌词(网易云网络歌词)+ 播放交互优化(2026-08-07 凌晨)

### 在线歌词

- 网关早已有 `/esp/v1/tracks/:id/lyrics`(上游 /lyric → LRC 解析为 `{plain, synced:[{time_ms,text}]}`),固件此前未接;现在接通:
  - 播放页检测到曲目变化时自动拉取该曲歌词,存 PSRAM(80 行上限,内部 RAM 几乎不占)
  - 播放页详情行显示当前句,逐句切换,交互与本地播放器一致
  - 状态:加载中显示"正在加载歌词…",未找到显示"未找到歌词,点击重试",失败显示"歌词加载失败,点击重试"(点击详情行重试);有歌词前显示歌手名
- 与本地歌词(文件内嵌 ID3 SYLT + lrclib.net)是两套独立存储/状态,互不干扰

### 播放交互优化(随上一批修复)

- 点歌立即进入播放页(不再等 resolve),页面显示"正在连接…",失败直接显示原因
- 播放页 LIST 槽打开当前队列来源列表,新增 `showKeepingStack` 保留页面栈,滑动返回回到播放页而非主页
