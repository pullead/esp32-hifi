...
## 追加:网关"唤醒不了/连接不上"根因与修复(2026-08-06 晚)

### 现象

- 电台(纯 HTTP)正常播放,但 设置 → 在线音乐 一直"音乐服务暂时不可用",在线音乐各分类全部连接失败;电脑端访问同一网关完全正常

### 根因:内部 RAM 耗尽, TLS 握手无法分配内存

- 通过设备 websocket `/test` 实测空闲内部堆仅 **13,327 字节**;mbedTLS 握手需要约 40KB 连续内部堆 → `WiFiClientSecure` 每次连接都分配失败 → 健康检查/浏览/播放全部失败
- 今天新功能把内部 RAM 占满了:云控制器栈 20K + 唤醒栈 20K + 榜单/新歌静态数组 ~8K + 卡住的诊断任务 24K,叠加在原已很紧的内存上
- 电台正常是因为纯 HTTP 不需要 TLS 缓冲区;电脑正常是因为没有内存约束
- 诊断任务(DNS/TCP/TLS 三阶段)自身也会在堆不足时表现异常,首次诊断任务因此卡死

### 修复(全部已实测)

1. **云端结果数组全部迁到 PSRAM**(`ps_malloc`):搜索 10 条 + 热门歌单 8 条 + 歌单曲目 20 条 + 排行榜 8 条 + 新歌 12 条 ≈ 24KB 内部静态 RAM → 释放给 TLS;setup() 分配失败时控制器/访问器优雅降级为 Error
2. **任务栈回到实测安全值**:cloudCtrl 20K→12K、cloudWake 20K→8K(原 8K 曾正常工作;真正的播放崩溃真因是延迟导航修复,不是栈)
3. **诊断接口升级**:`/cloud_diag_start` + `/cloud_diag_json` 逐步上报(先报 heap,再 DNS→TCP443→TLS 健康检查),栈降到 16K,方便定位任何一段的失败
4. 实测结果:空闲堆 13.3K → **54.7K**;设备端 `health_ok:true`(DNS 216.24.57.7 解析正常、TCP443 通、TLS 握手成功);静态 RAM 30.1%(98,652),比今日基线 34.7% 还低

### 结论

- 网关重新部署本身没问题(新域名解析到 Cloudflare CDN 216.24.57.x,PC 访问正常);失败全在设备端内存
- 后续加新功能务必把"内部 RAM / 空闲堆"当一等公民:大数据放 PSRAM,任务栈保持实测值,不盲目加大

## 追加 2:浏览 502(双冷启动)+ 防睡眠 + 列表乱码/排版(2026-08-06 深夜)

### 浏览 502 根因:双服务冷启动

- Render 免费层会把**网关和上游 api-enhanced 各自**在闲置 ~15 分钟后休眠;健康检查只唤醒网关,上游仍冷 → 浏览请求被网关秒回 502(上游 10s 超时等不到 30-90s 冷启动),设备重试窗口不够
- 修复:
  - 网关新增 `GET /esp/v1/wake`(90s 长超时等上游真正就绪,顺带预热 hot 缓存)
  - 固件唤醒任务两段式:健康检查(~70s)→ `/esp/v1/wake` 预热上游(~190s 总预算),"已连接"即保证浏览可用
  - 在线音乐首页进入也触发唤醒

### 防睡眠(按用户要求实现)

- 新增 `cloudKeepaliveTask`:每 9 分钟(低于 Render 15 分钟闲置阈值)后台做一次健康检查 + `/esp/v1/wake`,让网关与上游持续在线,首次打开不再等冷启动
- 注意:保持常驻会消耗 Render 免费层月度实例时长配额(750h/月,24x7 约 720h,基本贴线),若月底被暂停需升付费或改用外部监控(UptimeRobot 等)

### 列表乱码 + 排版

- 真 bug:手写 JSON 解析器不处理 `\u` 代理对(emoji 等非 BMP 字符)→ 半个代理对被编成非法 UTF-8 → 乱码;已补代理对合并为 4 字节 UTF-8
- 字体是 Noto Sans SC 13px 子集(ASCII + ~1.9 万 CJK/假名),无 emoji 字形 → 新增 `cloudTextSanitize()` 在解析时剥离非 BMP 与控制字符,不再显示豆腐块
- 排版:云列表原来 32px 行放两行 17px 行高的文字(上下对齐)会重叠,且 VIP 徽标压在文字上;统一改为本地列表同款 40px 行 + LEFT_MID ±10 双行布局,徽标右中对齐(-10),有徽标时文字宽度收窄到 205 避让
