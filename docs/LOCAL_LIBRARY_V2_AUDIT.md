...
开机
 ...
 上限 kMaxLocalTracks = 300                                 main.cpp:1614
       │
       ▼
...
| 项 | 实际情况 | 对方案的影响 |
|---|---|---|
| **曲库上限** | `kMaxLocalTracks = 300` | ⚠️ 方案假设 1000~5000，**必须先扩容** |
| **记录大小** | ≈322 B，`ps_calloc` 到 PSRAM | 2000 首 = 644KB；5000 首 = 1.6MB |
| **PSRAM 余量** | 正常模式启动后约 **2.58 MB** 空闲 | 2000 首舒服，5000 首紧张 |
| **内部 SRAM 余量** | LVGL 起来后仅约 **23 KB**，最大块 9216 B | 🔴 TLS 会话是最大威胁，见风险 R1 |
...
- **内部 SRAM 只剩 23 KB**，SQLite 的 page cache 无处安放
...
Arduino `String` 走普通 heap，**在内部 SRAM 只剩 23KB 的前提下这是硬约束**。
...
| # | 风险 | 说明 / 缓解 |
|---|---|---|
| **R1** | **内部 SRAM 只剩 23KB**，TLS 会话要 ~40KB | 🔴 最高。下载必须在**没在播放**或**播放本地**时进行；网络电台时暂停。必须实测 |
...
| **R5** | `String` 装整个 HTTP 响应 | 靠限制单次 20 条控制。超过就会撞 R1 |
