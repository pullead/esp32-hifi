...
AudioToneSettings playerCoreToneSettings() {
    ...
    settings.low = static_cast<int8_t>(std::clamp<int16_t>(s_tone.LP, -12, 12));
    settings.mid = static_cast<int8_t>(std::clamp<int16_t>(s_tone.BP, -12, 12));
    settings.high = static_cast<int8_t>(std::clamp<int16_t>(s_tone.HP, -12, 12));
    settings.balance = static_cast<int8_t>(std::clamp<int16_t>(s_tone.BAL, -16, 16));
    return settings;
}

void playerCoreSetToneSettings(const AudioToneSettings& settings, bool persist) {
    s_tone.LP = std::clamp<int16_t>(settings.low, -12, 12);
    s_tone.BP = std::clamp<int16_t>(settings.mid, -12, 12);
    s_tone.HP = std::clamp<int16_t>(settings.high, -12, 12);
    s_tone.BAL = std::clamp<int16_t>(settings.balance, -16, 16);
    setI2STone();
...
void playerCoreReadSnapshot(PlayerSnapshot* snapshot) {
    ...
    snapshot->vuLevel = audio.getVUlevel();
