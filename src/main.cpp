...
void setup() {
    ...
    // TEMP-DIAG v3 (remove after display bring-up): blink the backlight for 30 s straight,
    // plain GPIO (active-low per schematic: IO14 -> AO3401 P-FET gate -> LED_A).
    gpio_reset_pin((gpio_num_t)TFT_BL); // detach any LEDC/matrix routing from the pin
    pinMode(TFT_BL, OUTPUT);
    getTFT().fillScreen(TFT_RED);
    MWR_LOG_INFO("DIAG v3: BL blink 15x (1s on / 1s off), red fill");
    for (int i = 0; i < 15; i++) {
        digitalWrite(TFT_BL, LOW);  // ON
        vTaskDelay(1000);
        digitalWrite(TFT_BL, HIGH); // OFF
        vTaskDelay(1000);
    }
    digitalWrite(TFT_BL, LOW); // leave ON
    MWR_LOG_INFO("DIAG v3 done, BL left LOW (on)");
    // TEMP-DIAG v3 end
    ...
    defaultsettings();
    MWR_LOG_INFO("DIAG s_brightness after settings load: {}", s_brightness);
    ...
    for (uint8_t addr = 0; addr < 128; addr++) {
        ...
        if (twi->endTransmission() == 0) {
            if (addr == 0x18 || addr == 0x19) {
                ...
                i2c_items->cst816_found = true;
                i2c_items->cst816_addr = addr;
                if (log) MWR_LOG_WARN("cst816 candidate found at 0x{X}", addr);
            } else if (addr == 0x40) {
...
//---- TP_MODE ---------
    ...
    getTP().setRotation(TP_ROTATION);
    getTP().setMirror(TP_H_MIRROR, TP_V_MIRROR);
#elifdef TP_MODE_CST816 // CST816
    getTP().begin(&i2cBusOne, s_i2c_items.cst816_addr, s_h_resolution, s_v_resolution);
    getTP().setRotation(TP_ROTATION);
    getTP().setMirror(TP_H_MIRROR, TP_V_MIRROR);
#endif
}
