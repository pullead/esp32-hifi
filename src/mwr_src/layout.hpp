...
namespace layout_320x170 {

constexpr uint16_t h_res = 320, v_res = 170;
constexpr uint16_t h_footer = 20;
constexpr uint16_t hw_btn = 34;
constexpr uint16_t hw_btn_s = 28;
constexpr uint16_t w_vuMeter = 18;

constexpr uint16_t h_area = (v_res - 2 * h_footer) / 2;
constexpr uint16_t y_area2 = v_res - h_footer - h_area;
constexpr uint16_t y_btn = y_area2 + h_area / 2 + (h_area / 4 - hw_btn / 2);
constexpr uint16_t h_progBar = h_area / 2;
constexpr uint16_t y_progbar = y_area2;
...
constexpr coor winName = coor().pos(h_area, h_footer).size(h_res - h_area, h_area).pad(1, 1, 0, 0);
constexpr coor winProgbar = coor().pos(0, y_progbar).size(h_res, h_progBar).pad(5, 5, 0, 0);
...
constexpr coor winDigits = coor().pos(0, h_footer).size(h_res, y_btn - h_footer);
constexpr coor winWoHF = coor().pos(0, h_footer).size(h_res, 2 * h_area);

constexpr coor btnHP = coor().pos(8, h_footer + 0 * h_EQ).size(hw_btn_s, hw_btn_s);
constexpr coor btnBP = coor().pos(8, h_footer + 1 * h_EQ).size(hw_btn_s, hw_btn_s);
constexpr coor btnLP = coor().pos(8, h_footer + 2 * h_EQ).size(hw_btn_s, hw_btn_s);
constexpr coor btnBAL = coor().pos(8, h_footer + 3 * h_EQ).size(hw_btn_s, hw_btn_s);

constexpr coor sdrHP = coor().pos(45, h_footer + 0 * h_EQ).size(150, h_EQ).pad(0, 0, 5, 5);
constexpr coor sdrBP = coor().pos(45, h_footer + 1 * h_EQ).size(150, h_EQ).pad(0, 0, 5, 5);
constexpr coor sdrLP = coor().pos(45, h_footer + 2 * h_EQ).size(150, h_EQ).pad(0, 0, 5, 5);
constexpr coor sdrBAL = coor().pos(45, h_footer + 3 * h_EQ).size(150, h_EQ).pad(0, 0, 5, 5);

constexpr coor txtHP = coor().pos(200, h_footer + 0 * h_EQ).size(115, h_EQ).pad(0, 0, 1, 1);
constexpr coor txtBP = coor().pos(200, h_footer + 1 * h_EQ).size(115, h_EQ).pad(0, 0, 1, 1);
constexpr coor txtLP = coor().pos(200, h_footer + 2 * h_EQ).size(115, h_EQ).pad(0, 0, 1, 1);
constexpr coor txtBAL = coor().pos(200, h_footer + 3 * h_EQ).size(115, h_EQ).pad(0, 0, 1, 1);

inline constexpr uint8_t fonts[13] = {12, 13, 14, 16, 18, 20, 24, 28, 32, 38, 44, 54, 64};
...
#elifdef TFT_LAYOUT_XS
    using namespace layout_320x170;
